...
let UserService = class UserService {
    ...
    async login(user) {
        const foundedUser = await this.findByEmail(user.email);
        if (foundedUser) {
            const passwordsMathing = await this.authService.comparePasswords(user.password, foundedUser.password);
            if (passwordsMathing === true) {
                const payload = await this.findOne(foundedUser.id);
